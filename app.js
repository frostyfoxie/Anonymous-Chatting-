  ...
  ref.on('child_added',async snap=>{
    ...
    try{
      if(msg.encryptedText){
        ...
        div.innerHTML=`<div class="sender">${msg.username}</div><div class="bubble">${decrypted}</div>`;
      }
      else if(msg.encryptedFile){
        const decrypted=new Blob([await decryptData(msg.encryptedFile)]);
        const url = URL.createObjectURL(decrypted);
        div.innerHTML=`<div class="sender">${msg.username}</div><div class="bubble file"><a href="${url}" target="_blank">${msg.filename}</a></div>`;
      }
    }catch{
      div.innerHTML=`<div class="sender">${msg.username}</div><div class="bubble error">[Cannot decrypt]</div>`;
    }
